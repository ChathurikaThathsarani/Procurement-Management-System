...
const login = async (nic, password) => {
	try {
		...
		const { data } = await axios.post(api_base_url + "login", { nic, password }, config);
		console.log("----------------------------------------------------------------------------");
		console.log(data);
		return data;
	} catch (error) {
...
const register = async (name, dob, nic, gender, telephone, address, email, password, experience) => {
	const config = {
		headers: {
			"Content-type": "application/json",
		},
